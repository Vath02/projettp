...
//(seront utilisés lorsque l'on récupèrera des données à partir du formulaire) 
    ...
    public function createOrder() {
        ...
        $results = $this->pdo->db->prepare('INSERT INTO `c19v12_orders` (`number`, `status`, `id_c19v12_users`)
                VALUES (:number, :status, :id_c19v12_users)');
        // Associe une valeur à chaque marqueur nominatif de la requête
        $results->bindValue(':number', $this->number, PDO::PARAM_INT);
        $results->bindValue(':status', $this->status, PDO::PARAM_STR);
        $results->bindValue(':id_c19v12_users', $this->id_c19v12_users, PDO::PARAM_INT);
    ...
    public function getOrder() {
        ...
        $sql = 'SELECT `id`, `number`, `status` FROM `c19v12_orders` WHERE `id` = :id';
    ...
    public function getMaxNumber() {
        //Définition de la requête SQL
        $sql = 'SELECT MAX(`number`) AS `numberMax` FROM `c19v12_orders`';

        try {
            $result = $this->pdo->db->query($sql);
        } catch (Exception $e) {
            die('échec de la connexion :' . $e->getMessage());
        }

        //récupération de la valeur maximale de la colonne number
